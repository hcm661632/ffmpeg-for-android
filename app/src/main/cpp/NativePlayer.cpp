...
void JNICALL
Java_com_hua_nativeFFmpeg_NativeFFmpeg_nativePlay(JNIEnv *env, jobject instance,
                                                                    jstring input,
                                                                    jobject surface) {
    ...
    AVCodecParserContext *pAudioCodecParse = av_parser_init(pAudioCodec->id);
    if (!pAudioCodecParse) {
...
//                    usleep(1000 * 16);
        ...
        if (bestAudioStream == pkt->stream_index) {
            int ret = avcodec_send_packet(pAudioCodecCtx, pkt);
            if (ret < 0) {
...
void JNICALL
Java_com_hua_nativeFFmpeg_NativeFFmpeg_nativePlayAudioOnly(JNIEnv *env, jobject instance,
...
//    ALOGE("duration = %d",  pkt->duration);              // 368640
    ...
    while (av_read_frame(pFormatCtx, pkt) >= 0) {
        if (bestAudioStream == pkt->stream_index) {
            int ret = avcodec_send_packet(pAudioCodecContext, pkt);
            if (ret < 0) {
                ALOGE("Error while sending a packet to the decoder: %s\n",
                      av_err2str(ret));
            }
            ...
            while (ret >= 0) {
                ret = avcodec_receive_frame(pAudioCodecContext, frame);
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    break;
                } else if (ret < 0) {
                    ALOGE("Error while receiving a frame from the decoder: %s\n",
...
void JNICALL
Java_com_hua_nativeFFmpeg_NativeFFmpeg_nativePlayVideo_Recommand(JNIEnv *env, jobject instance,
                                                                 jstring input,
                                                                 jobject surface) {
    const char *file_name = env->GetStringUTFChars(input, 0);
    ALOGD("play");
...
void JNICALL
Java_com_hua_nativeFFmpeg_NativeFFmpeg_nativePlayVideo___1(JNIEnv *env, jobject instance,
                                                           jstring input,
                                                           jobject surface) {
    const char *file_name = env->GetStringUTFChars(input, 0);
    ALOGD("play");
